...
TODO(실제 링크 검증 필요): 이 접근은 DART API 문서상 알려진 동작을 근거로
구현했으나, 실제 감사보고서 뷰어 링크로 아직 검증되지 않았다. 특히
- 하루 공시량이 많아 max_pages(기본 20페이지=2000건)를 넘는 경우
- report_nm에 결산월이 명시되지 않아 _infer_bsns_year가 추정치를 반환하는 경우
는 실제 링크로 재확인이 필요하다.
"""
